...
int checker(nodo_t *temp, char *str, char *word, int len, int *cnt) {
    register int i, j, found;
    char *k2;

    if (temp -> right) checker(temp -> right, str, word, len, cnt);
    if (temp -> left) checker(temp -> left, str, word, len, cnt);
